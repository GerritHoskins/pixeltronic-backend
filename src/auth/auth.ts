...
export const register = async (req: Request, res: Response) => {
  ...
  try {
    hash(password, 10).then(async (hash: string) => {
      await User.create({
        username,
        password: hash,
      })
...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    await User.findOne({ username }).then((user) => {
      if (user?.password) {
        compare(password, user?.password).then((result: boolean) => {
          if (result) {
            const maxAge = 3 * 60 * 60;
            const token = sign({ id: user?._id, username, role: user?.role }, jwtSecretLive, {
              expiresIn: maxAge, // 3hrs in sec
        ...
        res.status(400).json({ message: 'Password missing or wrong. Login not succesful' });
      }
    });
  } catch (error) {
    console.error(error);
